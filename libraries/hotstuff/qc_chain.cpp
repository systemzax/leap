...
namespace eosio { namespace hotstuff {
   ...
   bool qc_chain::is_quorum_met(eosio::chain::quorum_certificate qc, extended_schedule schedule, hs_proposal_message proposal){
      ...
      if (qc.quorum_met == true ) {
         ...
         //ilog("qc : ${qc}", ("qc", qc));
         qc.quorum_met = evaluate_quorum(schedule, qc.active_finalizers, qc.active_agg_sig, proposal);
   ...
   bool qc_chain::am_i_proposer(){
      ...
      return prod_itr != _my_producers.end();
   }
...
/*              auto mf_itr = _my_producers.begin();
   ...
   void qc_chain::process_vote(hs_vote_message vote){
      ...
      if (!quorum_met){
         ...
         if (_current_qc.active_finalizers.size() > 1)
         ...
         if (quorum_met){
            ...
            if (_chained_mode==false && p_itr->phase_counter<3){
               ...
               if (_pending_proposal_block == NULL_BLOCK_ID)
                  proposal_candidate = new_proposal_candidate(p_itr->block_id, p_itr->phase_counter + 1);
               else
   ...
   void qc_chain::update_high_qc(eosio::chain::quorum_certificate high_qc){
      ...
      if (_high_qc.proposal_id == NULL_PROPOSAL_ID){
         ...
         if (old_high_qc_prop == _proposal_store.get<by_proposal_id>().end())
            return; //ilog(" *** CAN'T FIND OLD HIGH QC PROPOSAL");
         if (new_high_qc_prop == _proposal_store.get<by_proposal_id>().end())
            return; //ilog(" *** CAN'T FIND NEW HIGH QC PROPOSAL");
...
/*              ilog("=== _b_exec proposal #${block_num} ${proposal_id} block_id : ${block_id} phase : ${phase_counter} parent_id : ${parent_id}",
      ...
      if (_b_exec==NULL_PROPOSAL_ID){
         ...
         sequence_respected = last_exec_prop->get_height() < proposal.get_height();
