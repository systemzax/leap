...
public:
   ...
   void print_bp_state(name bp, std::string message){
      ...
      if (leaf_itr != qcc->second._proposal_store.get<qc_chain::by_proposal_id>().end())
         std::cout << "  - " << bp.to_string() << " current _b_leaf is : " << qcc->second._b_leaf.str() << " block_num : " << leaf_itr->block_num() << ", phase : " << unsigned(leaf_itr->phase_counter) << "\n";
      else
         std::cout << "  - No b_leaf value " << "\n";

      if (qc_itr != qcc->second._proposal_store.get<qc_chain::by_proposal_id>().end())
         std::cout << "  - " << bp.to_string() << " current high_qc is : " << qcc->second._high_qc.proposal_id.str() << " block_num : " << qc_itr->block_num() << ", phase : " << unsigned(qc_itr->phase_counter) <<  "\n";
      else
         std::cout << "  - No high_qc value " << "\n";

      if (lock_itr != qcc->second._proposal_store.get<qc_chain::by_proposal_id>().end())
         std::cout << "  - " << bp.to_string() << " current _b_lock is : " << qcc->second._b_lock.str() << " block_num : " << lock_itr->block_num() << ", phase : " << unsigned(lock_itr->phase_counter) <<  "\n";
      else
         std::cout << "  - No b_lock value " << "\n";

      if (exec_itr != qcc->second._proposal_store.get<qc_chain::by_proposal_id>().end())
         std::cout << "  - " << bp.to_string() << " current _b_exec is : " << qcc->second._b_exec.str() << " block_num : " << exec_itr->block_num() << ", phase : " << unsigned(exec_itr->phase_counter) <<  "\n";
      else
         std::cout << "  - No b_exec value " << "\n";
...
BOOST_AUTO_TEST_CASE(hotstuff_1) try {
   ...
   hotstuff_test_state ht;

   ht.initialize_qc_chains(tpm, {"bpa"_n}, unique_replicas);
...
BOOST_AUTO_TEST_CASE(hotstuff_3) try {
   ...
   ht.initialize_qc_chains(tpm, {"bpa"_n, "bpb"_n}, unique_replicas);

   ht.print_msg_queue_size(tpm);
